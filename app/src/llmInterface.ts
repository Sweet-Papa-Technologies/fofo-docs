...
import { colorize, getCostOfAPICall, getCostOfAPICallEmbedding, getCostOfAPICallTextOut, getTokens } from "./shared";
...
export async function infer(
  ...
  if (modelBackend === "OLLAMA") {
    ...
    const {totalTokens, totalBillableCharacters} = await genFunction.countTokens(request)
    ...
    const outputTokenData = await genFunction.countTokens(request)
  ...
  AIusageData.totalTokens += outputTokenData.totalTokens || 0;
  AIusageData.totalCharactersOut += outputTokenData.totalBillableCharacters || 0;
  AIusageData.totalCost += getCostOfAPICallTextOut(outputTokenData.totalBillableCharacters || 0);
